...
clusters <- skmeans(clusters, 8)
avg.common.cell.barcodes$cluster=clusters$cluster



#Add cluster to Seurat object
ST223.rna.singlets <- readRDS(file = "/vast/projects/Sisseq/ST223/ST223.rna.singlets.rds")
...
dt <- read.delim("ST223_common.barcodes.cpm.txt", header = TRUE)
library(umap)
library(ggplot2)

# 1. Recreate keep (force it to be logical)
keep <- rowSums(dt[,1:40]) > 0
keep <- as.logical(keep)

# 2. Prepare expression matrix
expr <- log2(dt[keep, 1:40] + 1)
...
avg.common.cell.barcodes$umap1 <- NA_real_
avg.common.cell.barcodes$umap2 <- NA_real_

# 5. Assign UMAP back
dt$umap1[keep] <- u$layout[,1]
dt$umap2[keep] <- u$layout[,2]

avg.common.cell.barcodes$umap1[keep] <- u$layout[,1]
avg.common.cell.barcodes$umap2[keep] <- u$layout[,2]

# 6. Plot
ggplot(avg.common.cell.barcodes[keep, ],
